...
if ar_file and ap_file:
    ...
    col2.metric("Lowest cash balance", f"{lowest_cash:,.2f}")
    col3.metric("Lowest cash date", lowest_cash_date.strftime("%d.%m.%Y"))
    col4.metric("Required financing", f"{required_financing:,.2f}")
    col5.metric("Final cash balance", f"{final_cash:,.2f}")
