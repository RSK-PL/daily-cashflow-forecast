...
def load_file(file, source_type):
    ...
    st.write(
        f"{source_type}: loaded {rows_after} rows out of {rows_before}, "
        f"total amount = {df['Amount'].sum():,.2f}"
    )

    if rows_after == 0:
        st.warning(f"{source_type} file was loaded, but no valid rows remained after cleaning dates/amounts.")
...
if not st.session_state.manual_events.empty:
    st.write("Manual planned events")
...
if ar_file and ap_file:
    ...
    st.subheader("CashEvents check")
    st.write(cash_events.groupby("Type")["Amount"].sum())
