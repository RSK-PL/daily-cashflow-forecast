...
if ar_file and ap_file:
    ...
    with col_end:
        kpi_end_date = st.date_input(
            "KPI end date",
            value=daily_cash["Date"].max().date(),
            min_value=daily_cash["Date"].min().date(),
            max_value=daily_cash["Date"].max().date()
        )

    if kpi_start_date > kpi_end_date:
        st.error("KPI start date cannot be later than KPI end date.")
        st.stop()

    kpi_daily_cash = daily_cash[
        (daily_cash["Date"].dt.date >= kpi_start_date) &
        (daily_cash["Date"].dt.date <= kpi_end_date)
    ].copy()

    previous_days = daily_cash[daily_cash["Date"].dt.date < kpi_start_date]

    if previous_days.empty:
        start_cash_balance = opening_cash
    else:
        start_cash_balance = previous_days["Cash balance"].iloc[-1]

    total_inflows = kpi_daily_cash["Daily inflow"].sum()
    total_outflows = kpi_daily_cash["Daily outflow"].sum()
    net_ar_ap_position = total_inflows - total_outflows
    net_cash_movement = kpi_daily_cash["Daily movement"].sum()
    final_cash = kpi_daily_cash["Cash balance"].iloc[-1]

    lowest_cash = kpi_daily_cash["Cash balance"].min()
    lowest_cash_date = kpi_daily_cash.loc[kpi_daily_cash["Cash balance"].idxmin(), "Date"]
    required_financing = abs(lowest_cash) if lowest_cash < 0 else 0

    highest_cash = kpi_daily_cash["Cash balance"].max()
    days_below_zero = (kpi_daily_cash["Cash balance"] < 0).sum()
    days_below_buffer = (kpi_daily_cash["Cash balance"] < minimum_cash_buffer).sum()

    biggest_daily_inflow = kpi_daily_cash["Daily inflow"].max()
    biggest_daily_inflow_date = kpi_daily_cash.loc[kpi_daily_cash["Daily inflow"].idxmax(), "Date"]

    biggest_daily_outflow = kpi_daily_cash["Daily outflow"].max()
    biggest_daily_outflow_date = kpi_daily_cash.loc[kpi_daily_cash["Daily outflow"].idxmax(), "Date"]

    st.subheader("Key liquidity KPIs")
    st.caption("KPIs are calculated for the selected date range.")

    col1, col2, col3, col4, col5 = st.columns(5)
    col1.metric("Start cash balance", f"{start_cash_balance:,.2f}")
    ...
    try:
        ...
        if selected_points:
            clicked_date = pd.to_datetime(selected_points[0]["x"]).date()
    except Exception:
        clicked_date = None

    with st.expander("Show receivables chart", expanded=False):
        fig_ar = go.Figure()
        fig_ar.add_trace(go.Bar(
            x=daily_cash["Date"],
            y=daily_cash["Daily inflow"],
            name="Receivables / AR"
        ))
        fig_ar.update_layout(
            title="Receivables / AR by date",
            xaxis_title="Date",
            yaxis_title="Amount",
            hovermode="x unified"
        )
    ...
    with st.expander("Show payables chart", expanded=False):
        fig_ap = go.Figure()
        fig_ap.add_trace(go.Bar(
            x=daily_cash["Date"],
            y=daily_cash["Daily outflow"],
            name="Payables / AP"
        ))
        fig_ap.update_layout(
            title="Payables / AP by date",
            xaxis_title="Date",
            yaxis_title="Amount",
            hovermode="x unified"
        )
    ...
    with st.expander("Show receivables vs payables chart", expanded=False):
        fig_ar_ap = go.Figure()
        fig_ar_ap.add_trace(go.Bar(
            x=daily_cash["Date"],
            y=daily_cash["Daily inflow"],
            name="Receivables / AR"
        ))
        ...
        fig_ar_ap.update_layout(
            title="Receivables vs Payables by date",
            xaxis_title="Date",
            yaxis_title="Amount",
            barmode="group",
            hovermode="x unified"
    ...
    default_document_date = clicked_date if clicked_date else daily_cash["Date"].min().date()

    selected_date = st.date_input(
        "Select date",
        value=default_document_date,
        min_value=daily_cash["Date"].min().date(),
        max_value=daily_cash["Date"].max().date(),
